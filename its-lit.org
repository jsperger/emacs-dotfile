...
I need to find a better way to access secrets than using =gptel=. The current
configuration means that I can't launch =aidermacs= until I've launched =gptel=, and
=aidermacs='s keybind won't appear until after =gptel= is loaded.
:PROPERTIES:
:CUSTOM_ID: setup-llmel
